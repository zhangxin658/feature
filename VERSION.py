...
class Worker(object):
    ...
    def numtofea(self, g, list):
        ...
        return feature1, feature2, my_fea1, my_fea2
    ...
    def is_better_self(self, kid_fit, g, my_fea):
        if kid_fit > self.my_max_fit:
            ...
            print(self.name, ' 第', g, '代进行更新')
            print(self.name, '更新为:', kid_fit)

    def is_better_sub(self, g):
        if max_fits[pop][0] < self.my_max_fit <= max_fits[pop][1]:
            max_fits[pop][0] = self.my_max_fit
            max_feas[pop][0] = self.my_max_fea.copy()
        elif self.my_max_fit > max_fits[pop][1]:
            i = max_fits[pop][1]
            j = max_feas[pop][1].copy()
            max_fits[pop][1] = self.my_max_fit
            max_feas[pop][1] = self.my_max_fea.copy()
            max_fits[pop][0] = i
            max_feas[pop][0] = j.copy()
        print(self.name, g, '更新pop种群分布')
        print('种群pop最大值为：', max_fits[pop])
    ...
    def could_reset_pop(self, g):
        ...
        val.sort()
        return random.random() < res and val[0] == self.fitness
    ...
    def get_large_fit(self, kid_fit1, kid_fit2, my_fea1, my_fea2):
    ...
    def work(self):
        for g in range(MAX_GLOBAL_EP):
            ...
            for kid in kids:  # 遍历每一个子代
                ...
                feature_Selected1, feature_Selected2, my_fea1, my_fea2 = self.numtofea(g+1, feature_list)
                # feature_set.append(my_fea)  # 将每个子代所选取的特征放到特征集合中
                ...
                kid_fit2 = self.get_fitness(data_sample2, trainy, data_predict2, testy,
                                            self.classifier)  # 然后更具所选的特征集合进行准确率求解
                kid_fit, my_fea = self.get_large_fit(kid_fit1, kid_fit2, my_fea1, my_fea2)
...
if __name__ == '__main__':
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        delte = 0.4
        numset = 12
        data = Dataset('E:/dataset/Sonar', SPLIT)
