    ...
    def __init__(self, name, data):
        with tf.variable_scope(name):
            ...
            self.loss = -tf.reduce_mean(
                self.mvn.log_prob(self.tfkids) * self.tfkids_fit + 0.001 * self.mvn.log_prob(
...
class chooseData(object):
    ...
    def _pailie_data(self, filename, divtype, labelloc):
        filename1 = filename + '.txt'
        if divtype == 0:
            self.dtype = ' '
        elif divtype == 1:
            self.dtype = ','
        if labelloc == 1:   #表示标签在最后一列
            numFeat = len(open(filename1).readline().split(self.dtype)) - 1
            self.size = numFeat + 1
            fr = open(filename1)
            for line in fr.readlines():
                xi = []
                curline = line.strip().split(self.dtype)
                self.label.append((curline[-1]))
                for i in range(0, numFeat):
                    xi.append(float(curline[i]))
                self.feature.append(xi)
        if labelloc == 0:  # 表示标签在最后一列
            numFeat = len(open(filename1).readline().split(self.dtype)) - 1
            self.size = numFeat + 1
            fr = open(filename1)
            for line in fr.readlines():
                xi = []
                curline = line.strip().split(self.dtype)
                self.label.append((curline[0]))
                for i in range(1, numFeat + 1):
                    xi.append(float(curline[i]))
                self.feature.append(xi)
        dataset = []
        dataset.append([i for i in range(len(open(filename1).readline().split(self.dtype)))])
        fr = open(filename1)
        i = 0
        for line in fr.readlines():
            xi = []
            xi.append(self.label[i])
            for val in self.feature[i]:
                xi.append(val)
            dataset.append(xi)
            i = i+1
        plfilename = filename + 'pl'
        print(np.array(dataset))
        self.saveData(plfilename, np.array(dataset))
        return self.size - 1

    def saveData(self, filename, dataname):
        myfilename = filename+'.txt'
        with open(myfilename, 'w') as file_object:  # 将文件及其内容存储到变量file_object
...
class divideData(object):
    ...
    def dividedata(self, workersize, type):
        ...
        for i in range(workersize):
            ...
            sample_name = 'E:/dataset/dorothea%d.txt' % i
...
class Worker(object):
    def __init__(self, name, data, numfea):
        self.name = name
        self.data = data
        self.numfea = numfea
...
if __name__ == '__main__':
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        # 首先进行的是将数据集同意格式化，格式为添加表头，并将标签移动到第一列，其中特征之间的划分采用空格的形式
        chdata = chooseData()
        filename = 'E:/dataset/arcene'
        numfea = chdata._pailie_data(filename, 1, 1)
        #将数据集划分完成之后，就通过mRMR算法对高维(特征数大于五百)的特征进行过滤
        if numfea > 10:
            filename = filename + 'pl'
            with tf.device("/gpu:0"):
                chdata._choose_fea(filename, 100)
            #加载过滤后的数据
            data = LoadData()
            filename = filename + 'ch' + '.txt'
            data._readData('split', filename, 2, 0.3)
            # 然后是首先创建全局网络
            ...
            for i in range(N_WORKERS):
                with tf.device('/gpu:%d' % i):
                    with tf.name_scope('GPU_%d' % i) as scope:
                        ...
                        workers.append(Worker(i_name, data, numfea))
