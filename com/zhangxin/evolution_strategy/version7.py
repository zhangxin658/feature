...
class chooseData(object):
    def __init__(self):
        self.size = 0
        self.dtype = ' ';
    def _choose_data(self, filename, type):
        if type == 1:
            self.dtype = ' '
        elif type == 2:
            self.dtype = ','
        numFeat = len(open(filename).readline().split(self.dtype))
        feature = []
        feature.append([i for i in range(len(open(filename).readline().split(self.dtype)))])
        fr = open(filename)
        ...
        self.saveData(filename, np.array(feature))
...
class divideData(object):
    ...
    def dividedata(self, workersize, type):
        ...
        for i in range(workersize):
            ...
            sample_name = 'E:/dataset/arcene%d.txt' % i
...
if __name__ == '__main__':
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        # chdata = chooseData()
        didata = divideData()
        didata._readData('E:/dataset/arcene.txt', 3, N_WORKERS)
        data = []
        for i in range(N_WORKERS):
            data.append(LoadData())
            filename = 'E:/dataset/arcene%d.txt' % i
            data[i]._readData('split', filename, 2, 0.3)
        print(data)
        ...
        global_pop = Global_pop('global', data[0])
        #接着就是定义一些所来对共性资源进行锁定
        ...
        for i in range(N_WORKERS):
            with tf.device('/gpu:%d' % i):
                with tf.name_scope('GPU_%d' % i) as scope:
                    ...
                    workers.append(Worker(i_name, idata, i))
