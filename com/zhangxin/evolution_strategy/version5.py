...
class Worker_pop(object):
    def __init__(self, name, data):
        with tf.variable_scope(name):
            ...
            with tf.variable_scope('mean'):
                self.mean = tf.Variable(tf.truncated_normal([self.DNA_size, ], stddev=0.01, mean=0.0), dtype=tf.float32,
                                   name=name + '_mean')
            with tf.variable_scope('cov'):
                self.cov = tf.Variable(10.0 * tf.eye(self.DNA_size), dtype=tf.float32, name=name + '_cov')
...
class LoadData(object):
    ...
    def getMatDataBysplit(self, x_train, x_test, y_train, y_test):
        ...
        self.saveData1('E:/dataset/trainX.txt', np.array(x_train))
        self.saveData1('E:/dataset/predictX.txt', np.array(x_test))
        self.saveData2('E:/dataset/trainy.txt', np.array(y_train))
        self.saveData2('E:/dataset/predicty.txt', np.array(y_test))
    ...
    def work(self):
        for g in range(MAX_GLOBAL_EP):
            ...
            for kid in kids:  # 遍历每一个子代
                ...
                feature_Select = self.numtofea(feature_list, data.getFeature())  # 将0，1串映射到特征的选取中
                feature_set.append(feature_list)  # 将每个子代所选取的特征放到特征集合中
                if data.getReadType() is 'split':
                    trainX_, testX_, trainy_, testy_ = data.getdata()
                    trainX = trainX_[0]
                    testX = testX_[0]
                    trainy = trainy_[0]
                    testy = testy_[0]
                ...
                elif data.getReadType() is 'nFold':
                    kid_fit = 0.0
                    for i in range(data.getTestsize()):
                        trainX_, testX_, trainy_, testy_ = data.getdata()
                        trainX = trainX_[i]
                        testX = testX_[i]
                        trainy = trainy_[i]
                    ...
                    kid_fit = kid_fit / data.getTestsize()
                    lock_max_fit.acquire()
                    max = global_pop.getMaxfit()
                    lock_max_fit.release()
                    kid_tifs = kid_fit - max
...
if __name__ == '__main__':
    ...
    # 创建读取数据的工具类
    data = LoadData()
    data._readData('split', 'E:/dataset/gisette0.txt', 1, 0.3)
    ...
    with tf.device("/cpu:0"):
        ...
        for i in range(N_WORKERS):
            ...
            workers.append(Worker(i_name, data))
